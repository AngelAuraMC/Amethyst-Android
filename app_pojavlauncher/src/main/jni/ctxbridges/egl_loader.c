...
bool dlsym_EGL() {
    ...
    bool nsbypass = getenv("POJAV_BYPASS_NAMESPACE") != NULL;
    char* eglName = (strncmp(gles ? gles : "", "libGLESv2_angle.so", 18) == 0) ? "libEGL_angle.so" : getenv("POJAVEXEC_EGL");
    if(nsbypass) {
        printf("PojavExec: loading libEGL %s through nsbypass\n", eglName);
    }
    void* dl_handle = loader_dlopen(eglName,"libEGL.so", RTLD_LOCAL|RTLD_LAZY, nsbypass);
